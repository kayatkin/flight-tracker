...
const ShareLinkOptions: React.FC<ShareLinkOptionsProps> = ({
  ...
  useEffect(() => {
    if (permissions === 'edit') {
      setInstructionsText(`Привет! Приглашаю тебя просмотреть и отредактировать мою историю перелётов.

📱 КАК ОТКРЫТЬ:
1. Установи Telegram (если нет)
2. Нажми на ссылку ниже в любом чате Telegram
3. Нажми кнопку "Open" или "Открыть"
4. Готово! Можешь редактировать мои перелёты
...
🌐 КАК ОТКРЫТЬ:
Просто открой эту ссылку в любом браузере (Chrome, Safari, Firefox) — всё откроется автоматически.
      ...
      <div className={styles.optionsSection}>
        ...
        <p className={styles.optionHint}>
          {shareWithInstructions 
            ? 'Ссылка будет отправлена с инструкцией как её открыть'
