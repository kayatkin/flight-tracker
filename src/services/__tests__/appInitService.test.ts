...
describe('initializeApp', () => {
  ...
  it('skips a matching share token after it was explicitly ignored for this session', async () => {
    const token = 'abc123token';
    window.history.pushState({}, '', `/?token=${token}`);
    ignoreInvitationTokenForSession(token);
    mockedAuthenticateOwner.mockResolvedValue({ userId: 'tg_owner', userName: 'Owner' });
    mockedLoadUserData.mockResolvedValue({
      flights: [],
      airlines: [],
      originCities: [],
    ...
    expect(mockedAuthenticateGuest).not.toHaveBeenCalled();
    expect(mockedAuthenticateOwner).toHaveBeenCalled();
    expect(result.appUser.isGuest).toBe(false);
    expect(result.userId).toBe('tg_owner');
  });
});
